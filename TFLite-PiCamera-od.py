...
total_persons1 = 0
total_persons_previous1 = 0
brkcnt = 0


text="WELCOME TO AI CUSTOMER COUNTER, please enter the store"
...
while True:
    ...
    t1 = cv2.getTickCount()
    frame = videostream.read()                                     #"""Grab frame from video stream """
        ...
        if classes1[i].all() == 0.0:
            ...
            if ((scores1[i] > MIN_CONF_THRESH) and (scores1[i] <= 1.0)):
                ...
                if object_name == 'person':
                    label = '%s: %d%%' % ('Person', int(scores1[i]*100)) # Example: 'person: 72%'
    ...
    # Loop over all detections and draw detection box if confidence is above minimum threshold
    total_persons_previous1 = total_persons1
    total_persons1 = 0
    ...
    for i in range(len(scores)):
        if classes[i].all() == 0.0:
            ...
            if ((scores[i] > MIN_CONF_THRESH) and (scores[i] <= 1.0)):
                ...
                if object_name == 'person':
                    label = '%s: %d%%' % ('Customer', int(scores[i]*100)) # Example: 'person: 72%'
                    labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2) # Get font size
                    label_ymin = max(ymin, labelSize[1] + 10) # Make sure not to draw label too close to top of window
                    cv2.rectangle(frame, (xmin, label_ymin-labelSize[1]-10), (xmin+labelSize[0], label_ymin+baseLine-10), (255, 255, 255), cv2.FILLED) # Draw white box to put label text in
                    cv2.putText(frame, label, (xmin, label_ymin-7), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2) # Draw label text
    ...
    if total_persons1 > total_persons_previous1:
        cnt_enter+= (total_persons1 - total_persons_previous1)

    if cnt_enter >=3 and brkcnt <2:
        brkcnt+=1
        cv2.rectangle(frame, (650,35), (1270,710), (0, 0, 255), 6)
        text="The store has reahced macimum capacity of 3 customers. Please wait in a line"
        os.system('espeak "'+text+'"')


    if cnt_enter >=3:
        cv2.rectangle(frame, (650,10), (1270,710), (0, 0, 255), 6)
    ...
    cv2.putText (frame,'Enterance',(780,680),cv2.FONT_HERSHEY_DUPLEX,3,(255,0,0),3,cv2.LINE_AA)
    cv2.putText (frame,'Exit',(10,680),cv2.FONT_HERSHEY_DUPLEX,3,(255,0,0),3,cv2.LINE_AA)
